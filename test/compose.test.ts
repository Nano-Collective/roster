...
test("parses a map with scalars, and types them", () => {
  const y = parseYaml(`org: playpip\nname: Pip\nprivate: true\ncount: 3\nmissing: null`);
  assert.deepEqual(y, { org: "playpip", name: "Pip", private: true, count: 3, missing: null });
});

test("parses nested maps", () => {
  const y = parseYaml(`human:\n  name: Will\n  github: will-lamerton\ntop: 1`);
  assert.deepEqual(y, { human: { name: "Will", github: "will-lamerton" }, top: 1 });
});
