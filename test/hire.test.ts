...
test("the app slug follows the house pattern rather than the org name", () => {
  ...
  assert.ok(p.warnings.some((w) => w.includes("pip-cto")), "an inferred value should say what it followed");
});
...
test("peer wiring goes both ways, and the label is the file owner's own", () => {
  ...
  try {
    ...
    assert.match(cfo, /- \{ handle: cto, brain: playpip\/technology, label: from-cfo \}/,
      "the new hire marks its own asks with from-cfo");

    const cto = readFileSync(join(root, "technology", "staff.yaml"), "utf8");
    assert.match(cto, /- \{ handle: cfo, brain: playpip\/finance, label: from-cto \}/,
...
test("a scaffolded hire is coherent: doctor passes and all three prompts compose", async () => {
  ...
  try {
    ...
    for (const [dir, handle, app] of [["technology", "cto", "pip-cto"], ["marketing", "cmo", "pip-cmo"]]) {
      mkdirSync(join(root, dir!), { recursive: true });
      writeFileSync(join(root, dir!, "staff.yaml"), [
        `handle: ${handle}`, `brain: playpip/${dir}`, `schedule: "0 7 * * 1-5"`,
