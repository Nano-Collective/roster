...
import { buildPlan, addToOrgYaml, wirePeers } from "../src/commands/hire.js";
...
function newOrg() {
  ...
  for (const [rel, text] of initFiles({
    org: "acme", name: "Acme Robotics", human: "someone", marker: "boss", opsName: "roster-ops",
  })) {
...
test("a new tenant gets the machinery, the org layer and a recorded base", () => {
  const files = initFiles({ org: "acme", name: "Acme", human: "someone", marker: "boss", opsName: "roster-ops" });
  for (const needed of [
    "org.yaml", "org/business.md", "org/voice.md", "org/guardrails.md", "org/operating.md",
    "compose.mjs", "runner-plan.mjs", ".github/workflows/session.yaml", ".roster-version",
  ]) {
...
test("business.md is questions, not invented prose", () => {
  ...
  const body = initFiles({ org: "acme", name: "Acme", human: "s", marker: "s", opsName: "roster-ops" })
    .get("org/business.md")!;
...
test("a brand new org can be hired into, and doctor is happy with the result", async () => {
  ...
  try {
    ...
    const plan = buildPlan(ws, org, "cto", {
      name: "Chief Technology Officer", dir: "technology",
      app: "acme-cto", publicApp: "acme-robot", statusIssue: 1,
    } as never, parseYaml);
    ...
    assert.deepEqual(failures.map((f) => `${f.scope}: ${f.title}`), [],
...
test("the first hire is told it has no shared identity rather than being given a made-up one", async () => {
  ...
  try {
    ...
    assert.ok(plan.warnings.some((w) => /no shared public identity/.test(w)),
...
test("a second hire copies what the first one established", async () => {
  ...
  try {
    ...
    const first = buildPlan(ws, readOrg(opsDir, parseYaml) as any, "cto", {
      name: "CTO", dir: "technology", app: "acme-cto", publicApp: "acme-robot", statusIssue: 1,
    } as never, parseYaml);
    ...
    const second = buildPlan(ws, readOrg(opsDir, parseYaml) as any, "cmo",
      { name: "CMO", dir: "marketing" } as never, parseYaml);

    assert.equal(second.staff.app, "acme-cmo", "the house naming pattern is followed");
    assert.equal(second.staff.publicApp, "acme-robot", "and the shared identity is genuinely shared");
    assert.deepEqual(second.peers.map((p) => p.handle), ["cto"]);
    assert.notEqual(second.staff.schedule, first.staff.schedule, "and they do not both start at once");
  } finally { rmSync(root, { recursive: true, force: true }); }
});
