...
bot: pip-cto[bot]
public_bot: pip-robot[bot]
public_token_env: PIPWEB_TOKEN
agent_secret: CLAUDE_CODE_OAUTH_TOKEN

identities:
  - { app: pip-cto, secret_prefix: CTO, scope: private }
  - { app: pip-robot, secret_prefix: BOT, scope: public }

works_in:
  - { repo: playpip/pip-web, role: contributor, checkout: true }
...
| Field | Means |
|---|---|
| `bot` | Login this staff member posts as on private trackers, as in `pip-cto[bot]`. |
...
authored by the App has author `pip-cto`. roster normalises this; if you are matching on it
...
`from-cto`, and that label lives on `playpip/marketing`, because that is where the CTO's asks
